...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            public void updateItemCount(int generation, int itemCount) {
                sendMessage(MessageThreadUtil.SyncQueueItem.obtainMessage(UPDATE_ITEM_COUNT, generation, itemCount));
            }

            @Override
            public void addTile(int generation, TileList.Tile<T> tile) {
                sendMessage(MessageThreadUtil.SyncQueueItem.obtainMessage(ADD_TILE, generation, tile));
            }

            @Override
            public void removeTile(int generation, int position) {
                sendMessage(MessageThreadUtil.SyncQueueItem.obtainMessage(REMOVE_TILE, generation, position));
            }
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            public void refresh(int generation) {
                sendMessageAtFrontOfQueue(MessageThreadUtil.SyncQueueItem.obtainMessage(REFRESH, generation, null));
            }

            @Override
            public void updateRange(int rangeStart, int rangeEnd,
                                    int extRangeStart, int extRangeEnd, int scrollHint) {
                sendMessageAtFrontOfQueue(MessageThreadUtil.SyncQueueItem.obtainMessage(UPDATE_RANGE,
                        rangeStart, rangeEnd, extRangeStart, extRangeEnd, scrollHint, null));
            }

            @Override
            public void loadTile(int position, int scrollHint) {
                sendMessage(MessageThreadUtil.SyncQueueItem.obtainMessage(LOAD_TILE, position, scrollHint));
            }

            @Override
            public void recycleTile(TileList.Tile<T> tile) {
                sendMessage(MessageThreadUtil.SyncQueueItem.obtainMessage(RECYCLE_TILE, 0, tile));
            }
    ...
    static class SyncQueueItem {
        ...
        static MessageThreadUtil.SyncQueueItem obtainMessage(int what, int arg1, int arg2, int arg3, int arg4,
                                                                                     int arg5, Object data) {
            synchronized (sPoolLock) {
                final MessageThreadUtil.SyncQueueItem item;
                if (sPool == null) {
    ...
    static class MessageQueue {
        ...
        synchronized MessageThreadUtil.SyncQueueItem next() {
            ...
            final MessageThreadUtil.SyncQueueItem next = mRoot;
            mRoot = mRoot.next;
            return next;
        }
        ...
        synchronized void sendMessage(MessageThreadUtil.SyncQueueItem item) {
            ...
            MessageThreadUtil.SyncQueueItem last = mRoot;
        ...
        synchronized void removeMessages(int what) {
            while (mRoot != null && mRoot.what == what) {
                MessageThreadUtil.SyncQueueItem item = mRoot;
                mRoot = mRoot.next;
                item.recycle();
            }
            if (mRoot != null) {
                MessageThreadUtil.SyncQueueItem prev = mRoot;
                MessageThreadUtil.SyncQueueItem item = prev.next;
                while (item != null) {
                    MessageThreadUtil.SyncQueueItem next = item.next;
                    if (item.what == what) {
                        prev.next = next;
                        item.recycle();
                    } else {
