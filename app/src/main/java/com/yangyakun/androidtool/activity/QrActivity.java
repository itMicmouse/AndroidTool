...
import com.yangyakun.androidtool.utils.ImageUtil;
import com.yangyakun.androidtool.utils.QRCodeUtil;
...
public class QrActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void saveImg(Bitmap bitmap){
        ...
        boolean isSaveSuccess = ImageUtil.saveImageToGallery(QrActivity.this, bitmap,fileName);
        if (isSaveSuccess) {
            Toast.makeText(QrActivity.this, "图片已保存至本地", Toast.LENGTH_LONG).show();
        } else {
            Toast.makeText(QrActivity.this, "保存图片失败，请稍后重试", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void imgChooseDialog(){
        AlertDialog.Builder choiceBuilder = new AlertDialog.Builder(QrActivity.this);
        choiceBuilder.setCancelable(false);
        choiceBuilder
                .setTitle("选择")
    ...
    private void takePhoto() {
        ...
        if (Build.VERSION.SDK_INT < 24) {
            ...
            imageUri = FileProvider.getUriForFile(QrActivity.this, "com.example.xch.generateqrcode.fileprovider", outputImage);
        }
        // 启动相机程序
    ...
    private void choosePhotoFromAlbum() {
        if (ContextCompat.checkSelfPermission(QrActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
